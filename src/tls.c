...
SEXP rnng_write_cert(SEXP cn, SEXP valid, SEXP inter) {
  ...
  int xc, exit = 1;
  if (interactive) nano_printf(1, "Generating key + certificate [    ]");
  ...
  if (interactive) nano_printf(1, "\rGenerating key + certificate [.   ]");
  ...
  if (interactive) nano_printf(1, "\rGenerating key + certificate [done]\n");
  exit = 0;
