...
#'
#' keyfile <- gen_key(filename = tempfile())
#' certfile <- cert_write(filename = tempfile(), key = keyfile, cn = "127.0.0.1")
...
#' @export
#'
cert_write <- function(filename = "cert.crt", key = "keyfile.key", cn = "localhost", valid = "20301231235959")
  invisible(.Call(rnng_cert_write, filename, key, cn, valid))
