...
#' Generate Key
#'
#' Generate a 4096 bit RSA private/public key pair in the PEM format using Mbed
#'     TLS; the first step in creating a self-signed certificate.
#'
#' @param filename [default 'keyfile.key'] character file path / name at which
...
#'
#' keyfile <- gen_key(filename = tempfile())
...
#' @keywords internal
#'
gen_key <- function(filename = "keyfile.key")
  invisible(.Call(rnng_gen_key, filename))

#' Write Certificate
#'
#' Create a self-signed certificate for use with encrypted TLS communications.
#'
#' @inheritParams self_signed_cert
#' @param key [default 'keyfile.key'] character file path / name to the issuer
#'     key file (as generated by \code{\link{gen_key}}).
#' @param filename [default 'cert.crt'] character file path / name at which
...
#'
#' keyfile <- gen_key(filename = tempfile())
#' certfile <- cert_write(key = keyfile, cn = "127.0.0.1", filename = tempfile())
#' cert <- readLines(certfile)
#'
#' unlink(certfile)
#' unlink(keyfile)
#' cert
...
#' @keywords internal
#'
cert_write <- function(key = "keyfile.key", cn = "localhost", valid = "20301231235959", filename = "cert.crt")
  invisible(.Call(rnng_cert_write, key, cn, valid, filename))
...
self_signed_cert <- function(cn = "localhost", valid = "20301231235959") {
  ...
  certfile <- cert_write(keyfile, cn = cn, valid = valid, filename = tempfile())
