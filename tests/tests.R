...
nanotesterr(msg_pipe(raio), "completed Aio")
...
nanotestn(cv_adjust(cv, 2L))
nanotest(cv_value(cv) == 2L)
nanotestn(cv_adjust(cv, -2))
nanotestz(cv_value(cv))
nanotestn(cv_adjust(cv, "a"))
nanotestz(cv_value(cv))
...
nanotesterr(cv_adjust(err, 1L), "valid")
...
nanotesterr(msg_pipe(fakesock), "valid Aio")
fakencurl <- `class<-`("test", "ncurlAio")
nanotesterr(msg_pipe(fakencurl), "valid Aio")
