library(nanonext)
nanotest <- function(x) invisible(x || stop())
nanotestn <- function(x) invisible(is.null(x) || stop())
nanotestnn <- function(x) invisible(!is.null(x) || stop())
nanotesterr <- function(x) invisible(tryCatch(x, error = function(e) TRUE) || stop())

n <- nano("pair", listen = "inproc://nanonext", autostart = FALSE)
n
n1 <- nano("pair", dial = "inproc://nanonext", autostart = FALSE)
n1
nanotest(inherits(n, "nanoObject"))
nanotest(inherits(n$socket, "nanoSocket"))
nanotest(inherits(n$socket, "nano"))
nanotest(is_nano(n))
n$newmethod <- "doesnotwork"
nanotestn(n$newmethod)

nanotest(is.integer(attr(n$socket, "id")))
nanotest(n$socket$state == "opened")
nanotest(n$socket$protocol == "pair")
nanotest(n$send("not ready") > 0L)
nanotest(n$recv() > 0L)
nanotest(n$socket_setopt("size", "recv-size-max", 8192) == 0L)
nanotest(n$socket_setopt("int", "recv-buffer", 8) == 0L)
nanotest(n$socket_setopt("string", "socket-name", "nano") == 0L)

n$listener[[1]]
nanotest(inherits(n$listener[[1]], "nanoListener"))
nanotest(n$listener[[1]]$url == "inproc://nanonext")
nanotest(n$listener[[1]]$state == "not started")
nanotest(n$listener_setopt("size", "recv-size-max", 1024) == 0L)
nanotest(n$listener_setopt("bool", "false", FALSE) == 9L)
nanotest(n$listener_setopt("int", "false", 100L) == 9L)
nanotest(n$listener_setopt("ms", "false", 100L) == 9L)
nanotest(n$listener_setopt("string", "false", "false") == 9L)
nanotest(n$listener_setopt("string", "false", NULL) == 9L)
nanotest(n$listener_setopt("uint64", "false", 100) == 9L)
nanotest(n$listener_start() == 0L)
nanotest(n$listener[[1]]$state == "started")

n1$dialer[[1]]
nanotest(inherits(n1$dialer[[1]], "nanoDialer"))
nanotest(n1$dialer[[1]]$url == "inproc://nanonext")
nanotest(n1$dialer[[1]]$state == "not started")
nanotest(n1$dialer_setopt("ms", "reconnect-time-min", 1000) == 0L)
nanotest(n1$dialer_setopt("size", "recv-size-max", 8192) == 0L)
nanotest(n1$dialer_setopt("bool", "false", FALSE) == 9L)
nanotest(n1$dialer_setopt("int", "false", 100L) == 9L)
nanotest(n1$dialer_setopt("string", "false", "false") == 9L)
nanotest(n1$dialer_setopt("string", "false", NULL) == 9L)
nanotest(n1$dialer_setopt("uint64", "false", 100) == 9L)
nanotest(n1$dialer_start() == 0L)
nanotest(n1$dialer[[1]]$state == "started")

nanotesterr(n$send(list(), mode = "raw"))
nanotesterr(n$recv(mode = "none"))
nanotesterr(n$recv(mode = "c"))
raio <- n$recv_aio(timeout = 1L)
raio
nanotest(is_error_value(call_aio(raio)$data))
nanotest(is_error_value(raio$data))
nanotestnn(n$send(data.frame(), block = NULL))
nanotestnn(n1$recv(block = FALSE, keep.raw = TRUE))
nanotestnn(n1$send("test", mode = "raw", block = 500))
nanotestnn(n$recv("character", block = 500, keep.raw = TRUE))
nanotestnn(n1$send(c(1L, 0L, 0L), mode = "r"))
nanotestnn(n$recv("int", keep.raw = TRUE))
saio <- n$send_aio(data.frame(), timeout = 500)
saio
nanotest(is_aio(saio))
nanotest(!is_nano(saio))
nanotestnn(saio[["aio"]])
nanotestnn(saio$result)
nanotesterr(n$send("wrong mode", mode = "none"))
raio <- n1$recv_aio(timeout = 500, keep.raw = TRUE)
raio$newfield <- "doesnotwork"
nanotestn(raio$newfield)
nanotest(is_aio(raio))
nanotestnn(raio[["aio"]])
nanotestnn(raio$raw)
nanotestnn(raio$data)
saio <- n$send_aio(c(1.1, 2.2), mode = "raw", timeout = 500)
saio$newfield <- "doesnotwork"
nanotestn(saio$newfield)
nanotest(is_aio(saio))
nanotest(is.logical(unresolved(saio)))
msg <- n1$recv_aio(mode = "numer", timeout = 500)
nanotestn(stop_aio(msg))
nanotestn(stop_aio(n))
nanotest(identical(call_aio(msg), msg))
nanotesterr(msg$data)
nanotest(identical(call_aio(n), n))
nanotest(!is_aio(n))
sraio <- n$send_aio(as.raw(0L), mode = "r", timeout = 500)
rraio <- n$recv_aio(mode = "r", timeout = 500)
nanotestnn(call_aio(rraio)$data)
sraio <- n$send_aio(as.raw(1L), mode = "ra", timeout = 200)
rraio <- n$recv_aio(mode = "ra", timeout = 200)
sraio <- n$send_aio(as.raw(2L), mode = "raw", timeout = 200)
rraio <- n$recv_aio(mode = "raw", timeout = 200)
sraio <- n$send_aio(1, mode = "raw", timeout = 200)
rraio <- n$recv_aio(mode = "nume", timeout = 200)
sraio <- n$send_aio(c(1, 2), mode = "raw", timeout = 200)
rraio <- n$recv_aio(mode = "numeri", timeout = 200)
sraio <- n$send_aio(c(1, 2, 3), mode = "raw", timeout = 200)
rraio <- n$recv_aio(mode = "numeric", timeout = 200)
sraio <- n$send_aio("hello", mode = 2L, timeout = 200)
rraio <- n$recv_aio(mode = "characte", timeout = 200)

xc <- n$dial(url = "inproc://two", autostart = TRUE)
nanotest(is.integer(xc))
nanotest(inherits(n$dialer[[1L]], "nanoDialer"))
xc <- n$listen(url = "inproc://three")
nanotest(is.integer(xc))
nanotest(inherits(n$listener[[2L]], "nanoListener"))

nanotest(close(n$listener[[1]]) == 0L)
nanotest(close(n$listener[[1]]) > 0L)
nanotest(close(n1$dialer[[1]]) == 0L)
nanotest(close(n1$dialer[[1]]) > 0L)
nanotest(n$close() == 0L)
nanotest(n1$close() == 0L)
nanotest(n1$close() > 0L)
nanotest(n$socket[["state"]] == "closed")
nanotest(n1$socket[state] == "closed")

n <- nano("req", listen = "inproc://testing")
n1 <- nano("rep", dial = "inproc://testing")
nanotest(n$socket_setopt("ms", "req:resend-time", 1000) == 0L)
nanotesterr(n$socket_setopt("none", "req:resend-time", 1000))
nanotesterr(n$socket_setopt("s", "req:resend-time", 1000))
ctx <- n$context()
ctx
ctx1 <- context(n1$socket)
nanotest(inherits(ctx, "nanoContext"))
nanotest(inherits(ctx, "nano"))
nanotest(is.integer(ctx$id))
nanotest(ctx$state == "opened")
nanotest(ctx$protocol == "req")
nanotest(setopt(ctx, "ms", "send-timeout", 1000) == 0L)
nanotest(setopt(ctx, "int", "false", 100L) == 9L)
nanotest(setopt(ctx, "bool", "false", FALSE) == 9L)
nanotest(setopt(ctx, "size", "false", 100L) == 9L)
nanotest(setopt(ctx, "string", "false", "false") == 9L)
nanotest(setopt(ctx, "string", "false", NULL) == 9L)
nanotest(setopt(ctx, "uint64", "false", 100) == 9L)
csaio <- send_aio(ctx, data.frame(), mode = "seria", timeout = 500)
nanotestnn(call_aio(csaio)$result)
nanotestnn(recv_aio(ctx1, timeout = 500))
nanotestnn(send(ctx, "context test", mode ="raw", block = 300))
nanotestnn(recv(ctx1, mode = "ch", block = 300))
nanotestnn(send(ctx, data.frame(), mode = "seri", block = 400))
msg <- recv_aio(ctx1, mode = "ser", timeout = 400, keep.raw = FALSE)
nanotest(is.logical(unresolved(msg)))
nanotestnn(call_aio(msg)$data)
nanotest(!unresolved(msg))
nanotestnn(send(ctx, c(TRUE, FALSE, TRUE), mode = 2L, block = 300))
msg <- recv_aio(ctx1, mode = 6L, timeout = 300, keep.raw = TRUE)
nanotestnn(call_aio(msg)$raw)
nanotestnn(msg$raw)
err <- send_aio(ctx1, "test")
stop_aio(err)
nanotesterr(err$result)
err <- send_aio(ctx1, "test")
nanotest(is_error_value(call_aio(err)$result))
nanotestnn(request(ctx, c(1+3i, 4+2i), recv_mode = "complex", timeout = 300, keep.raw = TRUE))
nanotestnn(reply(ctx1, execute = identity, send_mode = "ra", timeout = 300))
nanotest(close(ctx) == 0L)
nanotest(close(ctx) > 0L)
nanotest(ctx$state == "closed")
ctx2 <- n1$context()
nanotest(close(ctx2) == 0L)

pub <- nano("pub", listen = "inproc://ps")
sub <- nano("sub", dial = "inproc://ps")
nanotest(sub$socket_setopt(type = "bool", opt = "sub:prefnew", value = FALSE) == 0L)
nanotest(sub$socket_setopt(type = "uint64", opt = "false", value = 100) == 9L)
nanotest(sub$subscribe("test") == 0L)
nanotest(subscribe(sub$socket, NULL) == 0L)
nanotest(unsubscribe(sub$socket, NULL) == 0L)
nanotest(sub$unsubscribe("test") == 0L)
subctx <- sub$context()
nanotest(subscribe(subctx, 12) == 0L)
nanotest(unsubscribe(subctx, 12) == 0L)
nanotest(close(subctx) == 0L)
nanotest(sub$close() == 0L)
nanotest(pub$close() == 0L)

surv <- socket(protocol = "surveyor", listen = "inproc://sock1", dial = "inproc://sock2")
surv
resp <- socket(protocol = "respondent", listen = "inproc://sock2", dial = "inproc://sock1")
surv$dialer <- NULL
nanotestnn(surv$dialer)
nanotestnn(surv$listener)
nanotest(survey_time(surv, time = 5000) == 0L)
surctx <- context(surv)
nanotest(survey_time(surctx, time = 2000) == 0L)
nanotest(close(surctx) == 0L)
nanotest(close(surv) == 0L)
nanotest(close(resp) == 0L)
nanotest(is_error_value(recv(resp, keep.raw = TRUE)$raw))

nanotestnn(ncurl("https://www.r-project.org/"))
nanotestnn(ncurl("http://www.cam.ac.uk/"))
nanotestnn(ncurl("http://httpbin.org/post", convert = FALSE, method = "POST", headers = c(`Content-Type` = "text/plain"), data = "test", response = "server"))
nanotestnn(ncurl("http://httpbin.org/post", convert = FALSE, method = "POST", headers = list(`Content-Type` = "text/plain"), response = list("Date", "Server")))
nanotest(is_error_value(ncurl("http")$raw))
get <- ncurl("http://www.cam.ac.uk/", async = TRUE)
Sys.sleep(0.1)
nanotestnn(get$status)
put1 <- ncurl("http://httpbin.org/put", async = TRUE, method = "PUT", headers = c(Authorization = "Bearer token"), data = "test", response = c("date", "server"))
Sys.sleep(0.2)
put1$headers
put2 <- ncurl("http://httpbin.org/put", async = TRUE, method = "PUT", headers = list(Authorization = "Bearer token"), response = list("date", "server"))
put2
nanotestnn(put2$status)
nanotest(is.null(stop_aio(put2)))
nanotest(is_error_value(ncurl("http", async = TRUE)$data))
haio <- ncurl("http://i.i", async = TRUE)
Sys.sleep(0.1)
nanotestnn(haio$raw)
haio$data
nanotestnn(stream(dial = "tcp://127.0.0.1:5555"))
nanotest(stream(listen = "errorValue3") == 3L)
nanotesterr(stream())

nanotest(inherits(bus <- socket(protocol = "bus"), "nanoSocket"))
nanotest(inherits(push <- socket(protocol = "push"), "nanoSocket"))
nanotest(inherits(pull <- socket(protocol = "pull"), "nanoSocket"))
nanotest(inherits(survey <- nano(protocol = "surveyor"), "nanoObject"))
nanotest(inherits(respond <- nano(protocol = "respondent"), "nanoObject"))
nanotest(is_nano(bus))
nanotest(close(bus) == 0L)
nanotest(close(push) == 0L)
nanotest(close(pull) == 0L)
nanotest(survey$close() == 0L)
nanotest(respond$close() == 0L)
nanotesterr(socket(protocol = "newprotocol"))

nanotest(is.character(ver <- nng_version()) && length(ver) == 2L)
nanotest(is.character(nng_error(0L)))
nanotest(is_nul_byte(as.raw(0L)))
nanotest(!is_nul_byte(NULL))
nanotest(!is_error_value(1L))
nanotestnn(messenger("invalidURL"))
nanotest(is.double(mclock()))
nanotestn(msleep(1L))
nanotestn(msleep(1))
nanotestn(msleep("a"))
nanotest(is.double(random()))
nanotest(length(random(2)) == 2L)
nanotesterr(random("test"))
nanotestn(nano_init("test"))
nanotest(is_error_value(device(push, pull)))
nanotesterr(device(ctx, push))
nanotesterr(device(push, ctx))
for (i in c(100:103, 200:208, 226, 300:308, 400:426, 428:431, 451, 500:511))
  nanotest(is.character(status_code(i)))

fakesock <- `class<-`(new.env(), "nanoSocket")
nanotesterr(dial(fakesock))
nanotesterr(dial(fakesock, autostart = FALSE))
nanotesterr(listen(fakesock))
nanotesterr(listen(fakesock, autostart = FALSE))
nanotesterr(context(fakesock))
nanotesterr(close(fakesock))
fakectx <- `class<-`("test", "nanoContext")
nanotesterr(request(fakectx, data = "test"))
nanotesterr(close(fakectx))
fakestream <- `class<-`("test", "nanoStream")
fakestream
nanotesterr(send(fakestream, "test"))
nanotesterr(send_aio(fakestream, "test"))
nanotesterr(recv(fakestream))
nanotesterr(recv_aio(fakestream))
nanotesterr(setopt(fakestream, type = "int", opt = "test", value = "test"))
nanotesterr(close(fakestream))
fakedial <- `class<-`("test", "nanoDialer")
nanotesterr(start(fakedial))
nanotesterr(close(fakedial))
fakelist <- `class<-`("test", "nanoListener")
nanotesterr(start(fakelist))
nanotesterr(close(fakelist))
unres <- `class<-`(NA, "unresolvedValue")
unres
nanotest(unresolved(unres))
nanotest(identical(call_aio("a"), "a"))
nanotestn(stop_aio("a"))

nanotest(sha256("test") == "9f86d081884c7d659a2feaa0c55ad015a3bf4f1b2b0b822cd15d6c15b0f00a08")
nanotest(length(sha256(c("one", "two"), key = "PRIVATE_key", convert = FALSE)) == 32L)
nanotest(nchar(sha512(c(1.1, 2.2), as.raw(1L))) == 128L)
nanotest(length(sha512("testing", convert = FALSE)) == 64L)
nanotest(length(sha384(data.frame(), "KEY", convert = FALSE)) == 48L)
nanotest(nchar(sha384(7000L, key = NULL, convert = TRUE)) == 96L)
nanotest(length(sha224(c("one", "two"), key = "PRIVATE_key", convert = FALSE)) == 28L)
nanotest(sha224(as.raw(0L)) == "fff9292b4201617bdc4d3053fce02734166a683d7d858a7f5f59b073")
nanotest(base64enc("test") == "dGVzdA==")
nanotest(base64dec(base64enc("test")) == "test")
nanotest(is.raw(base64enc(data.frame(), convert = FALSE)))
nanotest(is.integer(unserialize(base64dec(base64enc(c(1L, 2L)), convert = FALSE))))
nanotesterr(base64dec("__"))

