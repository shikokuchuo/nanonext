...
nanotest(is.pairlist(nxt <- next_config(refhook = list(function(x) serialize(x, NULL), unserialize), class = "custom", vec = FALSE)))
nanotest(length(nxt) == 2L)
nanotest(is.function(nxt[[1L]]))
custom <- list(`class<-`(new.env(), "custom"), `class<-`(new.env(), "custom"))
nanotest(is.integer(req$send(custom, mode = 3L, block = 500)))
nanotest(is.environment(recv(rep, block = 500)[[1L]]))
nanotest(is.pairlist(nxt <- next_config(refhook = list(function(x) serialize(x, NULL), unserialize), class = "custom", vec = TRUE)))
