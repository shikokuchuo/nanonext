...
haio <- asyncurl("http://trin.cam.ac.uk/")
nanotest(length(call_aio(haio)$headers))
haio <- asyncurl("https://trin.cam.ac.uk/", convert = FALSE, response = "server")
nanotestnn(haio$status)
nanotest(is.integer(call_aio(haio)$status))
put1 <- asyncurl("http://postman-echo.com/put", method = "PUT", headers = c(Authorization = "Bearer token"), data = "test", response = c("Date", "server"), timeout = 3000L)
nanotestp(put1)
nanotest(is.integer(call_aio_(put1)$status))
nanotestnn(put1$headers)
nanotestnn(put1$data)
nanotestn(stop_aio(put1))
haio <- asyncurl("https://i.i")
nanotest(is_error_value(call_aio(haio)$data))
nanotestp(haio$data)
ncaio <- asyncurl("https://shikokuchuo.net/nanonext/reference/figures/logo.png")
nanotest(is.raw(call_aio(ncaio)$data) || is_error_value(ncaio$data))
nanotest(is_error_value(asyncurl("http")$data))
...
nanotesterr(asyncurl("https://", tls = "wrong"), "valid TLS")
...
nanotest(is_error_value(call_aio(asyncurl("https://www.cam.ac.uk/", tls = tls))$data))
...
if (requireNamespace("promises", quietly = TRUE)) {
  nanotestaio(n <- asyncurl("https://postman-echo.com/get"))
  nanotest(tryCatch(promises::is.promise(promises::then(n, cat)), error = function(e) TRUE))
  nanotest(promises::is.promising(call_aio(n)))
  nanotest(promises::is.promise(promises::as.promise(call_aio(asyncurl("https://postman-echo.com/get")))))
  later::run_now()
}
