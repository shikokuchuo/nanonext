...
nanotestnn(request(req$context, c(1+3i, 4+2i), recv_mode = "complex", timeout = 500, socket = req$socket, keep.raw = TRUE))
...
nanotestnn(cr <- request_signal(req$context, "test", timeout = 500, socket = req$socket, keep.raw = TRUE, cv = cv))
