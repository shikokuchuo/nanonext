...
nanotestaio(saio <- n$send_aio(data.frame(random(1e3L)), timeout = 900))
nanotestp(saio)
...
nanotestaio(cs <- request_signal(req$context, "test", cv = cv, timeout = 500))
...
nanotestaio(cs <- request(.context(req$socket), data = TRUE, send_mode = "raw", ack = TRUE))
nanotest(recv(ctxn, mode = "logical", block = FALSE))
nanotestz(send(ctxn, TRUE, mode = 0L, block = FALSE))
nanotestz(cv_reset(cv))
nanotestaio(cr <- recv_aio_signal(rep, cv = cv, mode = 8L))
nanotest(wait(cv))
nanotestz(reap(ctxn))
nanotestz(length(.subset2(cr, "data")))
