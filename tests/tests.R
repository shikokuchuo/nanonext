...
nanotestnn(n1$recv(block = FALSE))
nanotestz(n1$send("test", mode = "raw", block = 500))
nanotest(n$recv("character", block = 500) == "test")
nanotestz(n1$send(c(1L, 0L, 0L), mode = "r"))
nanotest(is.integer(n$recv("int")))
...
nanotestaio(raio <- n1$recv_aio(timeout = 500))
nanotestp(raio)
...
nanotesterr(call_aio(rraio)$data, "unserialization error")
nanotest(identical(recover_aio(rraio), rraio))
nanotest(is.raw(rraio$data))
...
nanotestaio(rraio <- n1$recv_aio(mode = "characte", timeout = 500))
...
nanotestaio(cr <- request_signal(req$context, "test", timeout = 500, cv = cv))
nanotestaio(cr <- recv_aio_signal(rep, timeout = 500, cv = cv))
...
nanotestp(haio$data)
ncaio <- ncurl("https://zenodo.org/record/8227727/files/shikokuchuo/mirai.promises-v0.1.2.zip?download=1", async = TRUE)
nanotest(is_error_value(tryCatch(call_aio(ncaio), error = function(e) `class<-`(e, "errorValue"))))
nanotest(is.raw(recover_aio(ncaio)$raw) || is_error_value(ncaio$raw))
...
unres <- as.symbol(" unresolvedValue ")
