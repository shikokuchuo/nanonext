...
nanotest(is.integer(req$send(serialize(NULL, NULL, ascii = TRUE), mode = 2L, block = 500)))
...
nanotest(is.integer(req$send(custom, mode = 3L, block = 500)))
nanotest(is.integer(recv(rep, block = 500)[[1L]]))
nanotest(is.pairlist(nxt <- next_config(refhook = list(function(x) raw(1L), as.integer), class = "custom", vec = TRUE)))
nanotest(is.integer(send(rep, custom, mode = 3L, block = 500)))
