...
nanotestz(send(req$socket, "", block = 500))
nanotestnano((p <- pipes(rep))[[1L]])
nanotestp(p[[1L]])
nanotestz(send(rep, "", block = 500))
nanotest(is.integer(pipes(req$socket, n = 1)[[1L]]$id))
...
nanotesterr(pipes(err), "valid Socket")
...
nanotestz(close(s1))
nanotestz(close(s))
nanotest(!identical(get0(".Random.seed"), {.rng_adv(); .Random.seed}))
