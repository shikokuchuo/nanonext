...
nanotestaio(saio <- n$send_aio(data.frame(random(1e3L), mode = 3L), timeout = 900))
nanotestp(saio)
...
nanotestaio(err <- send_aio(ctx, msg[["data"]], mode = "raw"))
...
nanotestaio(saio <- send_aio(ctx, msg[["value"]], mode = "next", timeout = 500))
nanotest(identical(req$recv(mode = "serial", block = 500), as.raw(1L)))
...
nanotest(typeof(ctxn <- .context(rep)) == "externalptr")
nanotestaio(cs <- request(.context(req$socket), data = TRUE, send_mode = "raw", autoclose = TRUE))
