...
nanotestaio(cr <- request_signal(req$context, "test", cv = cv, timeout = 500))
nanotestaio(cr <- recv_aio_signal(rep, cv = cv, timeout = 500))
nanotest(call_aio(cr)$data == "test")
nanotestaio(cr <- recv_aio_signal(context(rep), cv = cv, timeout = 500))
nanotestz(pipe_notify(rep, cv))
nanotestz(pipe_notify(req$socket, cv = cv, add = FALSE, remove = TRUE, flag = FALSE))
nanotestaio(cr <- request(.context(req$socket), data = TRUE, autoclose = TRUE))
nanotest(typeof(ctxn <- .context(rep)) == "externalptr")
nanotest(recv(ctxn, block = FALSE))
nanotest(send(ctxn, TRUE, block = FALSE))
