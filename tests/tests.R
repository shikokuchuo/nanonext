...
nanotestw(is.integer(call_aio(get)$status))
...
put2
nanotestn(call_aio(put2)$status)
nanotest(is.null(stop_aio(put2)))
nanotest(is_error_value(ncurl("http", async = TRUE)))
haio <- ncurl("http://i.i", async = TRUE)
nanotestw(is_error_value(call_aio(haio)$raw))
haio$data
...
nanotesterr(random("testing"))
nanotesterr(nano_init("testing"))
