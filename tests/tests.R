...
nanotestwn(ncurl("http://httpbin.org/post", convert = FALSE, method = "POST", headers = list(`Content-Type` = "text/plain"), request = list("Date", "Server")))
put1 <- ncurl("http://httpbin.org/put", async = TRUE, method = "PUT", headers = c(Authorization = "Bearer token"), data = "test", request = c("date", "server"))
nanotestn(put1$raw)
...
nanotest(length(sha256("test", key = NULL)) == 32L)
nanotest(is.raw(sha512(c(1.1, 2.2), as.raw(1L))))
nanotest(inherits(sha384(data.frame(), "KEY"), "nanoHash"))
nanotest(as.character(sha224(as.raw(0L))) == "fff9292b4201617bdc4d3053fce02734166a683d7d858a7f5f59b073")
