...
nng_version()
nanotestnano(n <- nano("pair", listen = "inproc://nanonext", autostart = FALSE))
nanotestnano(n1 <- nano("pair", dial = "inproc://nanonext", autostart = FALSE))
...
nanotestaio(raio <- n$recv_aio(timeout = 1L))
nanotestp(raio)
...
nanotestaio(saio <- n$send_aio(data.frame(random(1e4L)), timeout = 900))
nanotestp(saio)
...
nanotestaio(raio <- n1$recv_aio(timeout = 500, keep.raw = TRUE))
nanotestp(raio)
...
nanotestaio(msg <- n1$recv_aio(mode = "numer", keep.raw = TRUE, timeout = 500))
nanotest(identical(call_aio(msg), msg))
nanotestaio(msg <- n1$recv_aio(mode = "complex", keep.raw = TRUE, timeout = 500))
...
nanotestaio(rraio <- n1$recv_aio(mode = "r", timeout = 500))
...
nanotestw(is.raw(call_aio(rraio)$data))
nanotestaio(sraio <- n$send_aio("hello", mode = 2L, timeout = 500))
assign("aio", value = "test", envir = sraio)
nanotesterr(sraio$result, "valid")
nanotestaio(rraio <- n1$recv_aio(mode = "characte", keep.raw = TRUE, timeout = 500))
...
nanotest(recv(ctx, mode = "ch", block = 500) == "context test")
nanotestnn(req$send(data.frame(), mode = "seri", block = 500))
nanotestaio(msg <- recv_aio(ctx, mode = "ser", timeout = 500, keep.raw = FALSE))
nanotest(is.logical(.unresolved(msg)))
nanotest(is.logical(unresolved(msg)))
nanotest(is.list(call_aio(msg)$data))
nanotest(!unresolved(msg))
nanotest(is.integer(req$send(c(TRUE, FALSE, TRUE), mode = 2L, block = 500)))
nanotestaio(msg <- recv_aio(ctx, mode = 6L, timeout = 500, keep.raw = TRUE))
nanotest(is.raw(call_aio(msg)$raw))
...
nanotestaio(rek <- request(req$context, c(1+3i, 4+2i), recv_mode = "complex", timeout = 500, keep.raw = TRUE))
...
nanotestaio(cr <- request_signal(req$context, "test", timeout = 500, keep.raw = TRUE, cv = cv))
nanotestaio(cr <- recv_aio_signal(rep, timeout = 500, keep.raw = TRUE, cv = cv))
nanotest(is.raw(call_aio(cr)$raw))
nanotestaio(cr <- recv_aio_signal(context(rep), timeout = 500, cv = cv))
...
nanotest(is.integer(call_aio(relo)$status))
put1 <- ncurl("http://postman-echo.com/put", async = TRUE, method = "PUT", headers = c(Authorization = "Bearer token"), data = "test", response = c("date", "server"), timeout = 3000L)
nanotest(is.integer(call_aio(put1)$status))
nanotestnn(put1$headers)
nanotest(is.null(put1$raw) || is_error_value(put1$raw))
nanotestnn(put1$data)
put2 <- ncurl("http://postman-echo.com/put", async = TRUE, method = "PUT", headers = list(Authorization = "Bearer token"), response = list("date", "server"), timeout = 3000L)
nanotestp(put2)
...
if (length(s)) {
  ...
  nanotestaio(sr <- recv_aio(s, mode = "integer", keep.raw = TRUE, timeout = 500L, n = 8192L))
  nanotestw(length(call_aio(sr)[["raw"]]))
...
nanotest(length(sha384(as.list(random(1e4L)), "KEY", convert = FALSE)) == 48L)
...
nanotestnano(s <- socket(protocol = "req", listen = "inproc://nanorefhook"))
nanotestnano(s1 <- socket(protocol = "rep", dial = "inproc://nanorefhook"))
k <- new.env()
...
nanotest(is.function(register_refhook(function(x) if (typeof(x) == "weakref") base64enc(weakref_value(x)) else if (is.character(x)) unserialize(base64dec(x, convert = FALSE)))))
nanotestaio(send_aio(s, w))
nanotest(identical(recv(s1, block = 500), v))
nanotest(is.function(register_refhook()))
nanotestn(register_refhook(NULL))
nanotestz(close(s1))
nanotestz(close(s))
