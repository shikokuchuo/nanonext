...
nanotestaio(saio <- n1$send_aio(Reduce(strcat, replicate(5, random(1e3L))), mode = 3L, timeout = 900))
nanotestp(saio)
...
nanotest(is.logical(nanonext:::.unresolved(saio)))
...
nanotest(is.logical(nanonext:::.unresolved(msg)))
...
nanotesti(aio_collect(msg), aio_collect_(msg))
...
nanotest(is_error_value(aio_collect(err)))
nanotest(is_error_value(aio_collect(list(data = err))[["data"]]))
nanotest(is_error_value(aio_collect_(list(err))[[1L]]))
...
nanotest(!nanonext:::.unresolved(fakesock))
fakectx <- `class<-`("test", "nanoContext")
nanotest(!unresolved(fakectx))
nanotest(!nanonext:::.unresolved(fakectx))
...
nanotesterr(aio_collect_("a"), "object is not an Aio or list of Aios")
nanotesterr(aio_collect_(list("a")), "object is not an Aio or list of Aios")
nanotesterr(aio_collect(list(fakesock)), "object is not an Aio or list of Aios")
