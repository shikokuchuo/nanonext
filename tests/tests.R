...
raio
nanotestw(is_error_value(call_aio(raio)$data))
nanotest(is_error_value(raio$data))
nanotestwn(n$send(data.frame(), block = NULL))
...
nanotestwn(recv_aio(ctx1, timeout = 500))
...
nanotestwn(send(ctx, c(TRUE, FALSE, TRUE), mode = 2L, block = 300))
msg <- recv_aio(ctx1, mode = "l", timeout = 300, keep.raw = TRUE)
nanotestn(msg$raw)
nanotestwn(request(ctx, c(1+3i, 4+2i), recv_mode = "complex", timeout = 300, keep.raw = TRUE))
...
nanotestw(is_error_value(recv(resp, keep.raw = TRUE)$raw))

nanotestwn(ncurl("https://www.r-project.org/"))
nanotestwn(ncurl("http://www.cam.ac.uk/"))
nanotestwn(ncurl("http://httpbin.org/post", convert = FALSE, method = "POST", headers = c(`Content-Type` = "text/plain"), data = "test", response = "server"))
...
nanotestw(is_error_value(ncurl("http")$raw))
get <- ncurl("http://www.cam.ac.uk/", async = TRUE)
Sys.sleep(0.1)
nanotestwn(get$status)
...
put2
nanotestwn(put2$status)
nanotest(is.null(stop_aio(put2)))
nanotestw(is_error_value(ncurl("http", async = TRUE)$data))
