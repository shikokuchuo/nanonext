...
relo <- ncurl_aio("https://trin.cam.ac.uk/", convert = FALSE, response = "server")
nanotest(is.integer(call_aio(relo)$status))
relo <- ncurl_aio("https://trin.cam.ac.uk/", convert = FALSE, response = list("server"))
nanotest(is.integer(call_aio(relo)$status))
...
nanotestp(haio$data)
ncaio <- ncurl_aio("https://zenodo.org/record/8227727/files/shikokuchuo/mirai.promises-v0.1.2.zip?download=1")
...
nanotest(is_error_value(call_aio(ncurl_aio("https://www.cam.ac.uk/", tls = tls))$status))
