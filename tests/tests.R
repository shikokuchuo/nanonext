...
nanotestw <- function(x) invisible(suppressWarnings(isTRUE(x)) || {print(x); stop("the above was returned instead of TRUE")})
...
nanotestaio <- function(x) invisible(is_aio(x) || {print(x); stop("the above was returned instead of an Aio object")})
nanotestnano <- function(x) invisible(is_nano(x) || {print(x); stop("the above was returned instead of an object of class 'nano'")})
...
nanotestp <- function(x) invisible(is.character(capture.output(print(x))) || stop("print output of expression could not be captured as a character value"))
nanotestxp <- function(x) invisible(typeof(x) == "externalptr" || {print(x); stop("the above was returned instead of an external pointer")})
...
nanotest(n$send("not ready", mode = "serial") == 8L)
nanotest(n$recv() == 11L)
...
nanotestaio(saio <- n1$send_aio(paste(replicate(5, random(1e3L)), collapse = ""), mode = 1L, timeout = 900))
nanotestp(saio)
...
nanotestp(p <- tryCatch(collect_pipe(r), error = function(e) NULL))
...
nanotestaio(err <- send_aio(ctx, msg[["data"]], mode = "serial"))
...
nanotestaio(rek <- request(req$context, c(1+3i, 4+2i), send_mode = "serial", recv_mode = "serial", timeout = 500))
nanotestz(reply(ctx, execute = identity, recv_mode = 1L, send_mode = 1L, timeout = 500))
nanotest(is.complex(call_aio(rek)[["data"]]))

nanotest(is.list(cfg <- serial_config(class = "custom", sfunc = function(x) raw(1L), ufunc = as.integer, vec = FALSE)))
nanotest(length(cfg) == 4L)
...
nanotestz(send(ctxn, TRUE, mode = 1L, block = 500))
nanotestz(reap(ctxn))
...
nanotestz(close(rep))
if (is_nano(p)) nanotest(reap(p) == 12L)
if (is_nano(p)) nanotestw(close(p) == 12L)
...
nanotest(inherits(poly <- socket(protocol = "poly"), "nanoSocket"))
nanotestnano(bus)
...
if (call_aio(haio)$status == 200L) nanotestnn(haio$headers)
...
if (put1$status == 200L) nanotestnn(put1$headers)
if (put1$status == 200L) nanotestnn(put1$data)
...
if (call_aio(ncaio)$status == 200L) nanotest(is.raw(ncaio$data))
...
nanotesterr(ncurl("https://www.example.com/", tls = "wrong"), "valid TLS")
...
nanotest(is.character(ver <- nng_version()))
nanotest(length(ver) == 2L)
...
nanotest(is.raw(md5 <- nanonext:::md5_object("secret base")))
nanotest(length(md5) == 32L)
nanotest(is.double(mclock()))
nanotestn(msleep(1L))
nanotestn(msleep(1))
nanotestn(msleep("a"))
nanotest(is.character(urlp <- parse_url("://")))
nanotest(length(urlp) == 10L)
...
is_nano(s) && {
  ...
  nanotestnn(call_aio_(sr)[["data"]])
  nanotest(cv_value(cv) > 0L)
  nanotest(is.character(opt(s, "ws:request-headers")))
  nanotestnn(opt(s, "tcp-nodelay") <- FALSE)
  nanotesterr(recv(s, mode = "none", block = FALSE), "should be one of character")
  nanotesterr(recv(s, mode = "c", block = FALSE), "should be one of character")
...
nanotestp(unres)
nanotest(is.logical(unres <- unresolved(list("a", "b"))))
nanotest(length(unres) == 1L)
nanotest(is.integer(unres <- .unresolved(list("a", "b"))))
nanotest(length(unres) == 1L)
...
nanotest(length(cert) == 2L)
nanotest(is.character(cert[[1L]]))
nanotesti(names(cert), c("server", "client"))
nanotestxp(tls <- tls_config(client = cert$client))
nanotest(inherits(tls, "tlsConfig"))
nanotestp(tls)
nanotest(is_error_value(ncurl("https://www.example.com/", tls = tls)$status))
nanotest(is_error_value(call_aio(ncurl_aio("https://www.example.com/", tls = tls))$data))
nanotesterr(ncurl_session("https://www.example.com/", tls = cert$client), "not a valid TLS")
sess <- ncurl_session("https://www.example.com/", tls = tls)
nanotest(is_ncurl_session(sess) || is_error_value(sess))
if (is_ncurl_session(sess)) nanotest(is_error_value(transact(sess)[["headers"]]))
...
nanotestz(close(s1))
nanotestz(close(s))
if (promises) nanotestnano(s <- socket(listen = "inproc://nanonext"))
if (promises) nanotestnano(s1 <- socket(dial = "inproc://nanonext"))
if (promises) nanotestaio(r <- recv_aio(s, timeout = 500L))
if (promises) nanotest(promises::is.promise(promises::then(r, identity, identity)))
if (promises) nanotest(is.integer(send(s1, "promises test\n", block = 500L)))
if (promises) nanotestaio(r2 <- recv_aio(s, timeout = 1L))
if (promises) nanotest(promises::is.promising(call_aio(r2)))
if (promises) nanotest(promises::is.promise(promises::then(r2, identity, identity)))
if (promises) nanotest(promises::is.promising(call_aio(r)))
if (promises) nanotestaio(n <- ncurl_aio("https://postman-echo.com/get"))
if (promises) nanotest(promises::is.promise(promises::then(n, identity, identity)))
...
if (promises) nanotestz(close(s1))
if (promises) nanotestz(close(s))
if (promises) later::run_now()
