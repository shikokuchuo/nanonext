...
nanotesterr(msg_pipe(raio), "successful Aio")
...
nanotesterr(msg_pipe(msg), "active")
...
nanotestxp(p <- msg_pipe(rraio))
nanotest(is.integer(attr(p, "id")))
nanotestp(p)
nanotestz(close(p))
...
nanotesterr(cv_value(err) <- 0L, "valid")
nanotesterr(cv_reset(err), "valid")
nanotesterr(cv_value(cv) <- 0)
...
nanotesterr(msg_pipe(fakesock), "valid recvAio")
fakencurl <- `class<-`("test", "ncurlAio")
nanotesterr(msg_pipe(fakencurl), "valid recvAio")
