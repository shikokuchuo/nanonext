...
nanotestxp(req$context_open(verify = FALSE))
...
nanotesterr(request_signal(req$context, "test", cv = err), "valid")
...
nanotest(wait(cv))
nanotest(wait(cv2))
nanotestxp(sig <- signal(cv, time = 50L))
nanotest(!wait(cv))
nanotestxp(sig <- signal(cv2, time = 50, flag = FALSE))
nanotest(wait(cv2))
nanotestp(sig)
