...
A pre-release version of 'libnng' v1.6.0 (722bf46) is bundled built automatically during package installation.

A system installation of 'libmbedtls' will be used where detected in the usual filesystem locations. Mbed TLS versions v2.5.0 or newer are supported.

'INCLUDE_DIR' and 'LIB_DIR' environment variables may be specified prior to package install to specify custom locations.

#### Windows

For R >= 4.2 using the 'rtools42' toolchain, selecting source installation will allow the library sources to be downloaded and built as on Unix-alikes.

For previous R versions, pre-compiled 'libnng' v1.6.0 (722bf46) and 'libmbedtls' v3.2.1 libraries are downloaded and used instead.
