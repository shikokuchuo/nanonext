...
Note: it is recommended for stability and performance reasons to use the bundled library versions. To always compile the libraries from source even if system installations exist, set the environment variable `NANONEXT_LIBS` prior to installation e.g. by `Sys.setenv(NANONEXT_LIBS = 1)`.

Alternatively, 'libnng' is available as libnng-dev (deb) or nng-devel (rpm), and 'libmbedtls' is available as libmbedtls-dev (deb) or libmbedtls-devel (rpm).
