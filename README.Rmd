...
Installation from source requires 'libnng' >= v1.6.0 and 'libmbedtls' >= 2 - suitable installations are automatically detected - or else 'cmake' to compile 'libnng' v1.6.0 pre-release (8e1836f) and 'libmbedtls' v3.2.1 [v3.4.0 in dev version] included within the package sources.
...
For R >= 4.2 using the 'Rtools42' or 'Rtools43' toolchains, 'libnng' v1.6.0 (8e1836f) and 'libmbedtls' v3.2.1 [v3.4.0 in dev version] will be automatically compiled from the package sources during installation.

For previous R versions, pre-compiled 'libnng' v1.6.0 (8e1836f) and 'libmbedtls' v3.2.1 [v3.4.0 in dev version] libraries are downloaded and used for installation instead.
