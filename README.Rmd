...
Provides the interface for code and processes to communicate with each other - [receive data generated in Python, perform analysis in R, and send results to a C++ program](https://shikokuchuo.net/nanonext/articles/nanonext.html#cross-language-exchange) – all on the same computer or on networks spanning the globe.
...
- [sha1|224|256|384|512](https://shikokuchuo.net/nanonext/articles/nanonext.html#cryptographic-hashing) - cryptographic hash and HMAC algorithms
- [base64enc|dec](https://shikokuchuo.net/nanonext/articles/nanonext.html#cryptographic-hashing) - base64 encoding and decoding
...
Installation from source requires 'libnng' >= v1.5.0 and 'libmbedtls' >= 2.5.0 (suitable installations are automatically detected), or else 'cmake' to compile 'libnng' v1.6.0 and 'libmbedtls' v3.5.1 included within the package sources.
...
For R >= 4.2 using the 'Rtools42' or 'Rtools43' toolchains, 'libnng' v1.6.0 and 'libmbedtls' v3.5.1 will be automatically compiled from the package sources during installation.

For previous R versions, pre-compiled 'libnng' v1.6.0 and 'libmbedtls' v3.5.1 libraries are downloaded and used for installation instead.
