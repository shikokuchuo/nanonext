...
[![nanonext status badge](https://shikokuchuo.r-universe.dev/badges/nanonext?color=3f72af)](https://shikokuchuo.r-universe.dev/nanonext)
...
R binding for NNG (Nanomsg Next Gen), a successor to ZeroMQ. NNG implements Scalability Protocols, a reliable, high-performance messaging standard. Provides for common communications needs including publish/subscribe, request/reply and service discovery, over in-process, IPC, TCP, WebSocket and secure TLS transports.

As its own threaded concurrency framework, offers an intuitive toolkit for asynchronous programming and distributed computing, utilising 'aio' objects which resolve automatically upon completion, and synchronisation primitives which allow R to wait upon concurrent messaging threads.
...
Provides the interface for code and processes to communicate with each other - receive data generated in Python, perform analysis in R, and send results to a C++ program – all on the same computer or on networks spanning the globe.
...
Installation from source requires 'libnng' >= v1.5.0 and 'libmbedtls' >= 2.5.0 (suitable installations are automatically detected), or else 'cmake' to compile 'libnng' v1.6.0 alpha (a54820f) and 'libmbedtls' v3.5.0 included within the package sources.
...
For R >= 4.2 using the 'Rtools42' or 'Rtools43' toolchains, 'libnng' v1.6.0 alpha (a54820f) and 'libmbedtls' v3.5.0 will be automatically compiled from the package sources during installation.

For previous R versions, pre-compiled 'libnng' v1.6.0 alpha (a54820f) and 'libmbedtls' v3.5.0 libraries are downloaded and used for installation instead.
...
- [Luke Tierney](https://homepage.stat.uiowa.edu/~luke/) for documenting R's serialization mechanism and [mikefc](https://github.com/coolbutuseless) for meticulous annotations in {serializer}, which led to the package's own implementation of a low-level interface to R serialisation.
